"""Quick bench for ``live_airborne_bb_reversal_v2`` — 1y BTC+ETH 1h sweep.

Identical to ``bench_live_airborne_quick.py`` except imports v2 strategy.
Use --trend-sma to override the SMA period (default 100).
"""
...
logger = logging.getLogger("bench_live_airborne_v2_quick")
...
def _load_panels(symbols, months, freq):
    ...
    for sym in symbols:
        ...
        if not path.exists():
            logger.warning("symbol %s not in cache, skipping", sym); continue
        ...
        logger.info("  %s @ %s: %d bars  [%s .. %s]",
                    sym, freq, len(panel), panel.index[0].date(), panel.index[-1].date())
        del p1m
    gc.collect()
    return selected


async def _run_combo(panels, stop, tp, cost_bps, trend_sma):
    from backtest.strategies.live_airborne_bb_reversal_v2 import LiveAirborneBbReversalV2
    strat = LiveAirborneBbReversalV2(
...
async def _main_async(args):
    ...
    print("\n" + "=" * 110)
    print(f"live_airborne_bb_reversal_v2  |  freq={args.freq}  |  {args.months}mo  |  "
          f"symbols={','.join(panels.keys())}  |  cost={args.cost_bps:.0f}bp  |  "
          f"trend_sma={args.trend_sma}")
    print("=" * 110)
    rows = []
    for stop, tp, label in combos:
        c0 = time.time()
        e = await _run_combo(panels, stop, tp, args.cost_bps, args.trend_sma)
...
def _parse(argv=None):
    p = argparse.ArgumentParser(prog="bench_live_airborne_v2_quick")
    ...
    p.add_argument("--trend-sma", type=int, default=100,
                   help="trend SMA period (default 100 bars)")
