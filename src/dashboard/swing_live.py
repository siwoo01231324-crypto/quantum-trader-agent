...
def window_sim_trades(
    ...
    윈도우 귀속은 **진입시각(entry_ts) 기준** — "어제/최근7일/30일" 은 그 날짜에
    *진입한* 거래를 보여준다(사용자 확정 2026-06-30). 진입은 전날이고 청산만 윈도우
    안인 거래는 진입날 윈도우에 속한다; 표의 일시 칸에 진입·청산 시각을 둘 다
    노출해 "청산만 어제"인 거래를 헷갈리지 않게 한다(_render 일시 칸).
    """
    rows: list[dict] = []
    for t in sim_trades:
