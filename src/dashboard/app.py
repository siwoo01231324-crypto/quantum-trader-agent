...
# 2026-06-24: 진입 슬리피지 앵커. 발화가는 1m 봉 마감값인데 주문 체결 땐 시장이
# 그만큼 움직여 불리하게 채워진다(실거래 fill 역산 n=124 평균: 롱 +0.04%·숏 +0.15%
# adverse, 숏이 ~4배 심함). sim 진입가를 발화가에서 방향별 이동(롱↑/숏↓)해 TP/SL
# 트리거를 실체결 앵커에 맞춘다. AXTI 같은 경계거래가 sim timeout↔real SL 로 뒤집히던
# 원인. **봉 소스(Binance 15m)는 안 건드림** — 봉 소스는 격차 원인이 아니었고(슬리피지
# +미체결이 본체), Bitget 전환은 대량 rate-limit 으로 폐기됨(2026-06-24 #478 revert).
AIRBORNE_SLIPPAGE_LONG = 0.0004   # +0.04% 롱 entry 상향
AIRBORNE_SLIPPAGE_SHORT = 0.0015  # +0.15% adverse → 숏 entry 하향

# 미체결(거래소 진입불가) 종목 — 적중 sim 집계에서 제외(표엔 'untradeable' 로 노출).
# RWA 토큰화주식 중 거래소가 40092 'service unavailable' 로 거절하고 진입 성공 0회인
# 종목(2026-06-24 ground-truth: SKHYNIX·SAMSUNG). isRwa·봉활동은 깨끗한 구분자가
# 아니라(SAMSUNG 봉이 거래된 KIOXIA 보다 활발) 거래소 거절 실측 기반 설정형 리스트.
# env AIRBORNE_UNTRADEABLE_SYMBOLS (콤마구분) 로 운영중 추가 가능. CBRS 는 1회 거절됐으나
# 3회 진입성공이라 거래가능 → 제외 안 함.
_DEFAULT_UNTRADEABLE = "SKHYNIXUSDT,SAMSUNGUSDT"
AIRBORNE_UNTRADEABLE_SYMBOLS: frozenset[str] = frozenset(
    s.strip().upper() for s in os.getenv(
        "AIRBORNE_UNTRADEABLE_SYMBOLS", _DEFAULT_UNTRADEABLE).split(",") if s.strip()
)
...
def _simulate_airborne_fire(
    ...
    *,
    ...
    Args:
        ...
        slip_long/slip_short: 진입 슬리피지 — sim 진입가를 발화가에서 방향별로
            이동(롱 +slip_long 상향, 숏 -slip_short 하향)해 TP/SL 트리거를 실체결
            앵커에 맞춘다. **기본 0.0 (무슬리피지) → 순수 함수 byte-identical**;
            production endpoint 가 측정 보정값(AIRBORNE_SLIPPAGE_*) 을 명시 전달.
    ...
    # 진입 슬리피지 앵커 — 발화가에서 불리한 방향으로 이동(롱↑/숏↓).
    fire_close = float(fire["fire_close"])
    entry = fire_close * (1 + slip_long) if is_long else fire_close * (1 - slip_short)
...
# ── confluence 필터 (live-macross-regime-v1 리서치 숏-집중 스택, v0.6.87) ──
# CROSS 레코드(ts/cross/close/sma_slow)로 *서버에서 바로* 계산 가능한 부분집합:
...
def _ma_cross_confluence_reject(sim: dict) -> str | None:
    """숏-집중 confluence 필터 — 통과면 None, 탈락이면 사유 문자열.

    sim 은 ts/cross/close/sma_slow/btc_regime 필드를 가진 cross+sim dict.
    (ADX·기울기는 진입前 봉 필요 → 이 프록시 제외. 전략 on_bar 엔 적용됨.)
    """
    ...
    # KST 시간게이트
    try:
        ts = datetime.fromisoformat(str(sim.get("ts", "")).replace("Z", "+00:00"))
        hour_kst = ts.astimezone(_KST).hour
    except (ValueError, TypeError):
        hour_kst = None
    if hour_kst is None or hour_kst not in MA_CROSS_CONF_KST_HOURS:
        return f"kst_hour={hour_kst}(게이트밖)"
    close = sim.get("close")
    sma_slow = sim.get("sma_slow")
    if close is not None and sma_slow is not None and close > 0:
        # 자기 SMA200 하회 (숏은 가격이 자기 200선 아래여야)
        if not (close < sma_slow):
            return "self_sma200(자기200 위)"
        # 과확장 회피 (200선에서 10% 초과 아래로 이탈했으면 추격 금지)
        ext = (sma_slow - close) / close
        if ext > MA_CROSS_CONF_OVEREXTENSION_MAX:
            return f"overextended({ext*100:.0f}%>10%)"
    return None
...
<style>
.cal-head{display:flex;align-items:center;gap:10px;margin:26px 0 8px;flex-wrap:wrap}
.cal-head button{background:var(--surface);color:var(--text);border:1px solid var(--border);border-radius:4px;padding:4px 11px;cursor:pointer;font-family:var(--mono)}
.cal-head button:hover{border-color:var(--text2)}
.cal-title{font-weight:700;font-size:1rem;min-width:88px;text-align:center}
.cal-legend{font-size:.72rem;color:var(--text2)}
.cal-grid{display:grid;grid-template-columns:repeat(7,1fr);gap:4px}
.cal-wd{text-align:center;font-size:.72rem;color:var(--text2);padding:3px 0}
.cal-cell{min-height:64px;border:1px solid var(--border);border-radius:5px;padding:4px 5px;background:var(--surface);display:flex;flex-direction:column;gap:1px}
.cal-cell.cal-empty{border:none;background:transparent}
.cal-day{font-size:.68rem;color:var(--text2)}
.cal-pct{font-size:.92rem;font-weight:700;font-family:var(--mono);margin-top:auto}
.cal-n{font-size:.62rem;color:var(--text2)}
.cal-pos{background:rgba(34,197,94,.13);border-color:rgba(34,197,94,.42)}
.cal-pos .cal-pct{color:#22c55e}
.cal-neg{background:rgba(239,68,68,.13);border-color:rgba(239,68,68,.42)}
.cal-neg .cal-pct{color:#ef4444}
.cal-pct.cal-dim{color:var(--text2);font-weight:400}
.cal-sum{font-size:.74rem;color:var(--text2);margin-top:6px}
</style>
<div class="cal-head">
  <button onclick="calNav(-1)">◀</button>
  <span class="cal-title" id="cal-title">—</span>
  <button onclick="calNav(1)">▶</button>
  <button onclick="calNav('today')">이번달</button>
  <span class="cal-legend">📅 날짜별 net%(수수료 반영) · 초록=익절 빨강=손실 · 칸에 마우스 올리면 승률/PF</span>
</div>
<div id="calendar"><div class="empty">달력 로딩…</div></div>
<div class="cal-sum" id="cal-sum"></div>

<script>
const KST = 'Asia/Seoul';
const CAL_RULE = 'default';
let CAL_MONTH = null;
...
function renderFiresTable(sims){
  ...
  return `<div class="section-h2">🔥 개별 FIRE 상세 <span class="count">· ${rows.length}건 (최신순)</span></div>
    ...
    <div class="note">
      결과 — <b>TP</b>: 시뮬 +1.0% 익절. <b>SL</b>: 시뮬 −0.5% 손절. <b>SL_first</b>: 같은 15m 봉에서 high·low 가 둘 다 닿음 → 보수적으로 SL 부터 체결됐다고 가정. <b>timeout</b>: 4봉(=1h) 동안 TP/SL 둘 다 미도달 → 마지막 close 로 청산. <b>no_bars</b>: Binance fapi 봉 fetch 실패 / 너무 최근 fire 라 봉이 아직 안 닫힘. <b>untradeable</b>: 거래소가 진입 거절하는 종목(SKHYNIX·SAMSUNG 등 RWA 세션제한) — 실제로 못 사므로 승률/PF/sum 집계에서 제외(표엔 노출). <b>진입가</b>는 발화가에 측정 슬리피지(롱 +0.04%·숏 −0.15%)를 반영해 실체결 앵커에 맞춤.<br>
...
// ── 날짜별 적중 달력 (2026-06-24) ───────────────────────────────────────────
function _calCurMonth(){
  return new Intl.DateTimeFormat('en-CA',{timeZone:KST,year:'numeric',month:'2-digit'})
    .format(new Date()).slice(0,7);  // 'YYYY-MM'
}
function calNav(delta){
  if(delta==='today'){ CAL_MONTH=_calCurMonth(); loadCalendar(); return; }
  let [y,m]=CAL_MONTH.split('-').map(Number); m+=delta;
  if(m<1){m=12;y--;} if(m>12){m=1;y++;}
  CAL_MONTH=`${y}-${String(m).padStart(2,'0')}`;
  loadCalendar();
}
async function loadCalendar(){
  if(!CAL_MONTH) CAL_MONTH=_calCurMonth();
  const el=document.getElementById('calendar');
  try{
    const r=await fetch('/api/airborne_calendar?month='+encodeURIComponent(CAL_MONTH)+'&rule='+CAL_RULE);
    const j=await r.json();
    if(j.error){ el.innerHTML=`<div class="error">${esc(j.error)}</div>`; return; }
    renderCalendar(j);
  }catch(e){ el.innerHTML=`<div class="error">달력 로딩 실패: ${esc(String(e))}</div>`; }
}
function renderCalendar(j){
  document.getElementById('cal-title').textContent=j.month;
  const days=j.days||{};
  ...
  let monNet=0, monN=0, nDataDays=0;
  for(let d=1;d<=nDays;d++){
    const key=`${j.month}-${String(d).padStart(2,'0')}`;
    const x=days[key];
    if(x){
      const v=x.net_pct;
      const cls=v>0?'cal-pos':(v<0?'cal-neg':'');
      const wr=x.win_rate!=null?(x.win_rate*100).toFixed(0)+'%':'—';
      const pf=x.pf!=null?x.pf.toFixed(2):'—';
      monNet+=v; monN+=x.n; nDataDays++;
      cells+=`<div class="cal-cell ${cls}" title="${key} · ${x.n}건 · 승률 ${wr} · PF ${pf} · TP${x.tp}/SL${x.sl}/TO${x.timeout}">
        <div class="cal-day">${d}</div>
        <div class="cal-pct">${fmtPctRaw(v,1)}</div>
        <div class="cal-n">${x.n}건</div></div>`;
    } else {
      cells+=`<div class="cal-cell"><div class="cal-day">${d}</div><div class="cal-pct cal-dim">·</div></div>`;
    }
  }
  document.getElementById('calendar').innerHTML=`<div class="cal-grid">${cells}</div>`;
  document.getElementById('cal-sum').innerHTML=
    `${j.month} 합계: ${nDataDays}일 거래 · 일별 net 단순합 ${fmtPct(monNet,1)} · 총 ${monN}건`;
}
loadCalendar();
</script>
</body>
...
<div class="header-row">
  ...
  <span style="display:inline-flex;gap:0;border:1px solid var(--border);border-radius:4px;overflow:hidden;font-size:.74rem">
    ...
    <button class="regime-btn" data-regime="confluence" onclick="changeRegime('confluence')"
            style="background:var(--surface);color:var(--text);border:0;border-left:1px solid var(--border);padding:5px 10px;cursor:pointer;font-family:var(--mono)">confluence(숏-집중)</button>
  </span>
...
function renderExcluded(d){
  ...
  const rows = [...ex].sort((a,b) => String(b.ts || '').localeCompare(String(a.ts || '')));
  const trs = rows.map(r => {
    const crossCls = r.cross === 'death' ? 'is-short' : 'is-long';
    const crossTxt = r.cross === 'death' ? 'DEATH' : 'GOLDEN';
    return `<tr>
      ...
      <td><span class="side-badge ${crossCls}">${esc(crossTxt)}</span></td>
      <td class="td-num">${esc(fmtPrice(r.close))}</td>
      <td style="color:var(--red)">${esc(r.reject || '')}</td>
      <td class="td-num" style="color:var(--text3)">${r.pct != null ? fmtPct(r.pct, 2) : '—'}</td>
    </tr>`;
  }).join('');
...
def create_app(state: DashboardState | None = None) -> FastAPI:
    ...
    async def api_airborne_metrics(
        ...
        # ── 미체결(거래소 진입불가) 종목 분리 ────────────────────────────
        # SKHYNIX·SAMSUNG 처럼 거래소가 40092 로 진입 거절(RWA 토큰화주식 세션제한)
        # 하는 종목은 발화해도 실제로 못 산다 → 적중 통계(승률/PF/sum)를 부풀린다.
        # 집계에서 제외하고 표에는 'untradeable' 로 노출(no_bars 와 별개로 구분).
        untradeable_fires = [
            {**f, "outcome": "untradeable", "pct": None, "bar_idx": None}
            for f in fires
            if f.get("symbol", "").upper() in AIRBORNE_UNTRADEABLE_SYMBOLS
        ]
        fires = [
            f for f in fires
            if f.get("symbol", "").upper() not in AIRBORNE_UNTRADEABLE_SYMBOLS
        ]
            ...
            async with _httpx.AsyncClient() as client:
                async def _sim_one(fire: dict) -> dict | None:
                    ...
                    out = _simulate_airborne_fire(
                        ...
                        slip_long=AIRBORNE_SLIPPAGE_LONG,
                        slip_short=AIRBORNE_SLIPPAGE_SHORT,
                    )
                    if out is None:
                        return None
                    return {**fire, **out}
        ...
        payload = {
            ...
            "sims_total": len(sims),
            "untradeable_total": len(untradeable_fires),  # 거래소 진입불가(집계제외)
            **agg,
            # per-fire 상세 — untradeable 도 표엔 노출(집계엔 미반영)
            "sims": sims + no_bar_fires + untradeable_fires,
            "rule": {
                "name": rule,
    ...
    # ── airborne 적중 달력 — 월별 일자별 집계 (2026-06-24) ─────────────────────
    # window(오늘/7d/30d) 외에 "어느날 몇%" 를 달력으로 보기 위한 일자별 분해.
    # metrics 엔드포인트와 동일 sim 경로(슬리피지·미체결 제외 포함) 재사용 후
    # KST 날짜로 grouping → 일자별 _aggregate_airborne_sims.
    _airborne_calendar_cache: dict[str, dict[str, Any]] = {}

    @app.get("/api/airborne_calendar")
    async def api_airborne_calendar(
        month: str = Query(..., description="YYYY-MM (KST)"),
        rule: str = Query("default", description="default | 2pct"),
    ) -> JSONResponse:
        """월(KST) 단위 일자별 적중 집계. 응답: {month, days:{'YYYY-MM-DD':{n,sum_pct,
        net_pct,win_rate,pf,tp,sl,timeout}}, fires_total, rule, cached}."""
        import time as _time  # noqa: PLC0415
        import httpx as _httpx  # noqa: PLC0415
        from collections import defaultdict  # noqa: PLC0415

        if rule == "2pct":
            tp_pct, sl_pct, hold_bars = (
                AIRBORNE_TP_PCT_2PCT, AIRBORNE_SL_PCT_2PCT, AIRBORNE_HOLD_BARS_2PCT)
        else:
            rule = "default"
            tp_pct, sl_pct, hold_bars = (
                AIRBORNE_TP_PCT, AIRBORNE_SL_PCT, AIRBORNE_HOLD_BARS)

        # ── month → KST [start, end) ────────────────────────────────────
        try:
            y_str, m_str = month.split("-")
            y, m = int(y_str), int(m_str)
            month_start_kst = datetime(y, m, 1, tzinfo=_KST)
        except (ValueError, TypeError):
            return JSONResponse(
                {"error": f"month must be YYYY-MM, got {month!r}"}, status_code=400)
        month_end_kst = (datetime(y + 1, 1, 1, tzinfo=_KST) if m == 12
                         else datetime(y, m + 1, 1, tzinfo=_KST))
        since_utc = month_start_kst.astimezone(timezone.utc)
        # 미래로는 안 감 (지금까지만)
        until_utc = min(month_end_kst.astimezone(timezone.utc),
                        datetime.now(timezone.utc))

        now = _time.time()
        cache_key = f"{month}:{rule}"
        ce = _airborne_calendar_cache.get(cache_key)
        if (ce is not None and now - ce["ts"] < AIRBORNE_METRICS_CACHE_TTL
                and ce["data"] is not None):
            return JSONResponse({**ce["data"], "cached": True})

        # docker logs 백필 → store (metrics 와 동일)
        backfill_since = (datetime.now(timezone.utc) - timedelta(days=4)).strftime(
            "%Y-%m-%dT%H:%M:%SZ")
        live_fires = await asyncio.to_thread(
            _parse_airborne_fires_from_docker_logs, backfill_since)
        store = _get_airborne_fire_store()
        if live_fires:
            store.append_many(live_fires)
            ...
            async with _httpx.AsyncClient() as client:
                async def _sim_one(fire: dict) -> dict | None:
                    async with sem:
                        bars = await _fetch_15m_bars(client, fire["symbol"], fire["ts"])
                    out = _simulate_airborne_fire(
                        fire, bars, tp_pct=tp_pct, sl_pct=sl_pct, hold_bars=hold_bars,
                        slip_long=AIRBORNE_SLIPPAGE_LONG,
                        slip_short=AIRBORNE_SLIPPAGE_SHORT)
                    return {**fire, **out} if out is not None else None
                results = await asyncio.gather(
                    *(_sim_one(f) for f in missing_fires), return_exceptions=False)
            new_sims = [r for r in results if r is not None]
            if new_sims:
                sim_cache.put_many(new_sims)

        sims = cached_sims + new_sims
        # KST 날짜로 grouping → 일자별 집계
        by_day: dict[str, list[dict]] = defaultdict(list)
        for s in sims:
            try:
                d = datetime.fromisoformat(
                    str(s["ts"]).replace("Z", "+00:00")
                ).astimezone(_KST).strftime("%Y-%m-%d")
            except (ValueError, TypeError, KeyError):
                continue
            by_day[d].append(s)
        days: dict[str, dict] = {}
        for d, day_sims in by_day.items():
            a = _aggregate_airborne_sims(day_sims)
            days[d] = {
                "n": a["n"], "sum_pct": a["sum_pct"], "net_pct": a["net_pct"],
                "win_rate": a["win_rate"], "pf": a["pf"],
                "tp": a["tp"], "sl": a["sl"], "timeout": a["timeout"],
            }
        payload = {
            "month": month, "days": days, "fires_total": len(fires),
            "rule": {"name": rule, "tp_pct": tp_pct, "sl_pct": sl_pct,
                     "fee_pct": AIRBORNE_FEE_PCT},
            "cached": False,
        }
        _airborne_calendar_cache[cache_key] = {"ts": now, "data": payload}
        return JSONResponse(payload)
    ...
    async def api_ma_cross_metrics(
        ...
        regime: str = Query(
            ...
            description="raw(전체) | filtered(BTC 레짐 정렬: 골든+상승/데드+하락만) | "
                        "confluence(숏-집중: 데드+BTC하락+KST시간+자기200+과확장 근사)",
        ),
    ) -> JSONResponse:
        ...
        elif regime == "confluence":
            # 숏-집중 confluence 필터 — 통과분만 agg, 탈락분은 excluded 로 노출.
            passed: list[dict] = []
            for s in sims:
                rej = _ma_cross_confluence_reject(s)
                if rej is None:
                    passed.append(s)
                else:
                    excluded.append({
                        "ts": s.get("ts"), "symbol": s.get("symbol"),
                        "cross": s.get("cross"), "close": s.get("close"),
                        "btc_regime": s.get("btc_regime"),
                        "outcome": s.get("outcome"), "pct": s.get("pct"),
                        "reject": rej,
                    })
            sims = passed
        ...
        payload = {
            ...
            "regime": regime,
            "sims_total": len(sims),
            "excluded": excluded,            # confluence 에서 걸러낸 CROSS + 사유
