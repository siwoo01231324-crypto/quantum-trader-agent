...
# 옛 fire 재호출 시 캐시 hit → Bitget REST 0 회 (수십 초 절감).
# Rule key → AirborneSimCache 인스턴스. 룰별로 캐시 파일 분리해 결과 안 섞이게.
# 기본 룰(+1%/-0.5%) → logs/airborne_fires/sim_cache_bg1m_slip.jsonl
# 신규 +2%/-1% 룰     → logs/airborne_fires/sim_cache_2pct_bg1m_slip.jsonl
#
# 2026-06-24 캐시 파일명 bump (_bg1m_slip): 시뮬 모델을 실거래에 정합시키며 두 가지
# 변경 — (1) 봉 소스 Binance fapi 15m → Bitget history-candles 1m, (2) 진입
# 슬리피지 앵커 도입. 옛 파일의 Binance-15m·무슬리피지 outcome 은 실거래와
# 어긋난다(사용자 2026-06-24 검증: AXTI 는 진입 슬리피지로, MMT 는 15m 해상도로
# sim TP↔real SL 뒤집힘). dedup 구조라 한 번 박힌 결과는 안 갱신되므로 새 파일로
# 전량 재시뮬한다.
_AIRBORNE_SIM_CACHES: dict[str, AirborneSimCache] = {}

_AIRBORNE_SIM_CACHE_PATHS: dict[str, str] = {
    "default": "logs/airborne_fires/sim_cache_bg1m_slip.jsonl",
    "2pct": "logs/airborne_fires/sim_cache_2pct_bg1m_slip.jsonl",
}
...
# 2026-06-24: 봉 소스 Binance fapi 15m → Bitget history-candles 1m 전환.
#   - 거래소 일치: 실거래는 Bitget. Binance fapi 가격 경로가 다르고, 토큰화주식은
#     Binance fapi 에 없어 no_bars 로 빠지거나 엉뚱하게 시뮬됐다.
#   - 해상도: 15m 봉은 봉 내부 SL-먼저 순서를 못 잡아 MMT 처럼 sim TP↔real SL 뒤집힘.
#     1m 봉으로 intra-bar TP/SL 트리거 순서를 실거래 fill 과 정합.
#   - ⚠️ Bitget `candles` 는 옛 startTime 무시(최근 봉만 반환) → `history-candles` 사용.
#   hold_bars: 1m × 60 = 1h (기존 15m × 4 = 1h 와 동일 보유시간).
AIRBORNE_BAR_GRANULARITY = "1m"
AIRBORNE_BAR_MS = 60_000  # 1분 봉 길이(ms) — close_time/endTime 계산용
# 진입 슬리피지 앵커 — 실거래 fill 역산 측정(n=124, 2026-06-24) 기반 평균 adverse.
# 발화가는 1m 봉 마감값인데 주문 체결 땐 시장이 그만큼 움직여 불리하게 채워진다.
# 롱은 더 높게(+), 숏은 더 낮게(−) 체결 — 숏이 ~4배 심함(숏 중앙 +0.147% vs 롱 +0.025%).
# sim 진입가를 이만큼 이동시켜 TP/SL 트리거를 실체결 앵커에 맞춘다(AXTI flip 의 원인).
...
AIRBORNE_HOLD_BARS = 60   # 1m 봉 60개 = 1h
...
AIRBORNE_HOLD_BARS_2PCT = 60   # 1m 봉 60개 = 1h (2026-06-24 Bitget 1m 전환)
...
def _simulate_airborne_fire(
    ...
    *,
    ...
    Args:
        ...
        hold_bars: required # of bars for a "timeout" verdict (default = 60 = 1h).
            keyword-only — 기존 caller (positional 2 args) 는 그대로 default 룰 사용.
        slip_long/slip_short: 진입 슬리피지 — sim 진입가를 발화가에서 방향별로
            이동(롱 +slip_long 상향, 숏 -slip_short 하향)해 TP/SL 트리거를 실체결
            앵커에 맞춘다. **기본 0.0 (무슬리피지) → 순수 함수는 byte-identical**;
            production endpoint 가 측정 보정값(AIRBORNE_SLIPPAGE_*) 을 명시 전달.

    Returns:
        {outcome: "TP"|"SL"|"SL_first"|"timeout", pct: float, bar_idx: int}
        - pct: 손익비 (TP=+tp_pct, SL=-sl_pct, timeout=슬리피지조정 진입가 대비 변화율)
...
<div class="subtitle">qta-airborne-daemon 의 FIRE 라인을 docker logs 에서 파싱 → 영속 JSONL store 누적 → Bitget history-candles 1m 봉 시뮬(실거래 거래소·해상도 정합 + 진입 슬리피지 반영) → 적중률·PF·net% 집계. dashboard 가 처음 켜진 시점부터 store 에 누적되므로 docker rotation (4d 한도) 이후에도 옛 fire 보존.</div>
<div class="nav">
  <a href="/">← 대시보드</a>
  <a href="/cs-tsmom">cs-tsmom</a>
  <a href="/signals">신호 목록</a>
...
<div class="header-row">
  ...
  <div class="meta" id="meta">로딩 중…</div>
  <span class="rule-badge" id="rule-badge">룰: TP +1.0% / SL -0.5% / 60봉(1m=1h) hold · Bitget 1m · 진입슬리피지(롱+0.04%/숏-0.15%) · 양방향 수수료 0.034% (테더맥스 54% 페이백 반영)</span>
  <button class="refresh-btn" id="refresh-btn" onclick="forceRefresh()">↻ 캐시 무효화 + 재계산</button>
</div>
...
function renderFiresTable(sims){
  ...
  return `<div class="section-h2">🔥 개별 FIRE 상세 <span class="count">· ${rows.length}건 (최신순)</span></div>
    ...
    <div class="note">
      결과 — <b>TP</b>: 시뮬 +1.0% 익절. <b>SL</b>: 시뮬 −0.5% 손절. <b>SL_first</b>: 같은 1m 봉에서 high·low 가 둘 다 닿음 → 보수적으로 SL 부터 체결됐다고 가정. <b>timeout</b>: 60봉(=1h) 동안 TP/SL 둘 다 미도달 → 마지막 close 로 청산. <b>no_bars</b>: Bitget candles 봉 fetch 실패 / 너무 최근 fire 라 봉이 아직 안 닫힘. <b>진입가</b>는 발화가에 측정 슬리피지(롱 +0.04%·숏 −0.15%)를 반영해 실체결 앵커에 맞춤.<br>
...
def create_app(state: DashboardState | None = None) -> FastAPI:
    ...
    async def _fetch_airborne_bars(
        client: Any, symbol: str, fire_ts_iso: str,
        limit: int = AIRBORNE_HOLD_BARS,
    ) -> list[dict]:
        """Bitget v2 `history-candles` — fire 시점 이후 1m 봉 (실거래 거래소).

        2026-06-24: Binance fapi 15m → Bitget 1m 전환. 실거래가 Bitget 이라
        가격 경로가 정합하고(토큰화주식 포함), 1m 해상도로 봉 내부 TP/SL 순서를
        라이브 fill 만큼 잡는다.

        ⚠️ `/api/v2/mix/market/candles` 는 옛 startTime 을 무시하고 최근 봉만
        반환한다(사용자 2026-06-24 검증: 9시간 어긋남). 과거 fire 시뮬에는
        `history-candles` 를 startTime+endTime 으로 윈도우 고정해 사용한다.
        응답이 최신→과거로 올 수 있어 open_time 오름차순 정렬. 실패는 빈 list
        → caller 가 no_bars 처리.
        """
        try:
            ...
            return []
        # endTime 으로 윈도우를 hold_bars(+여유 5봉)만큼 닫아 정확한 구간만 받는다.
        end_ms = start_ms + (limit + 5) * AIRBORNE_BAR_MS
        try:
            r = await client.get(
                "https://api.bitget.com/api/v2/mix/market/history-candles",
                params={"symbol": symbol, "productType": "USDT-FUTURES",
                        "granularity": AIRBORNE_BAR_GRANULARITY,
        ...
        for b in rows:
            try:
                bars.append({
                    "open_time": int(b[0]), "open": float(b[1]),
                    "high": float(b[2]), "low": float(b[3]),
                    "close": float(b[4]),
                    "close_time": int(b[0]) + AIRBORNE_BAR_MS - 1,
                })
            except (IndexError, ValueError, TypeError):
                continue
    ...
    async def api_airborne_metrics(
        ...
        # cache miss 인 새 fire 들만 Bitget history-candles 1m 봉 fetch + 시뮬.
        ...
        if missing_fires:
            ...
            async with _httpx.AsyncClient() as client:
                async def _sim_one(fire: dict) -> dict | None:
                    async with sem:
                        bars = await _fetch_airborne_bars(client, fire["symbol"], fire["ts"])
