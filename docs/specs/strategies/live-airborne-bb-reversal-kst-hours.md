...
name: Live Airborne BB Reversal v1.2 Bidir (KST {1,2,3,5,6,7,8,23} Hours, v3+5h)
status: active
...
summary_ko: |
  ...
  airborne BB-reversal 시그널 + KST {1,2,3,5,6,7,8,23} 시 진입 게이트 (v3+5시).
...
# Live Airborne BB Reversal v1.2 Bidir — KST {1,2,3,5,6,7,8,23} Hours (v3+5h)
...
1. **시간 게이트**: `hour(close_ts_kst) ∈ {1,2,3,5,6,7,8,23}` (v3+5시) 아니면 hold.
...
라이브 에어본 포착분(`logs/airborne_fires/sim_cache_2pct.jsonl`, 6/04~, TP+2%/SL−1% sim)
시각별 PF 분석에서 KST 5시가 강세 → 양방향 공유 게이트에 추가.

| 구분 | n | 승률 | PF | 기대값 | 합계 |
|---|---|---|---|---|---|
| 5시 롱 | 63 | 66.7% | **2.22** | +0.32% | +20.3 |
| 5시 숏 | 28 | 39.3% | 1.23 | +0.11% | +3.0 |
| 현재 게이트 {1,2,3,6,7,8,23} 롱 평균 | 404 | 54.7% | 1.75 | +0.27% | +109.8 |

- 5시 롱 PF 2.22 로 기존 게이트 평균(1.75) 상회, 숏도 양수 → 공유 게이트 추가 무해.
- ⚠️ **인접 4시는 PF 0.13(−36.5)로 최악** — 절대 추가 금지, 5시만 선별.
- ⚠️ 5y hourly 로는 5시 롱 PF 0.99(≈본전) — 최근-윈도우 알파일 수 있어 모니터링 필요.
  실거래 5시 롱은 게이트에 막혀 n=1(데이터 없음)이라 판단 근거는 라이브 포착분.
