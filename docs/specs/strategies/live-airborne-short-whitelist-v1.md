...
- kst-19-hour-entry-gate-afternoon-excluded
summary_ko: >
...
### 2026-06-14 — 오후 역알파 시간대 제외 (24h → 19h, band-aid)

최근 22일(5/24~6/14) airborne SHORT **sim** 시간대별 PF 분석 결과, 명백한 역알파 시간대를 진입 게이트에서 제외:

| KST | n | WR% | PF | net%(fee후) | 판정 |
|---|--:|--:|--:|--:|---|
| 06 | 33 | 76 | 5.82 | +18.15 | 황금 (유지) |
| 10 | 38 | 79 | 6.69 | +21.48 | 황금 (유지) |
| 01·03·11·23 | — | 48~58 | 1.7~2.9 | + | 강세 (유지) |
| 04 | 39 | 36 | 1.01 | -1.23 | **제외** (무엣지+fee) |
| 13 | 27 | 33 | 0.92 | -1.58 | **제외** (음수) |
| **14** | 24 | **12** | **0.18** | **-9.14** | **제외 (명백 역알파)** |
| 16 | 31 | 35 | 1.02 | -0.87 | **제외** (무엣지+fee) |
| 17 | 32 | 34 | 1.07 | -0.45 | **제외** (무엣지+fee) |

→ `kst_entry_hours` = `{4,13,14,16,17}` 제외한 19시간. production.yaml authoritative.

**⚠️ 이건 근본해결이 아니다 (band-aid):** 위 5y bench(2026-06-08) 결론 3번이 "**게이트는 TP/SL 이 정상일 때만 도움. 1%/0.5% 에선 게이트도 못 살림(PF 0.586→0.592)**" 이라 기록함. 0.5% 손절이 노이즈에 털리는 게 근본결함. 본 trim 은 #380 라이브누적우선 방침하에서 **명백 역알파(특히 14시 PF 0.18)만 우선 차단**하는 모니터링 조치이며, 근본해결은 TP/SL 재설계(Hard-OOS 6%/3% 복귀)임을 재확인한다. 시간대 PF 는 sim(Binance 이상체결) 기준이라 실거래 슬리피지 미반영 — 라이브 누적으로 재검증 필요.

회귀 가드: `tests/configs/test_airborne_short_whitelist_hours.py`.
...
- id: live-airborne-short-whitelist-v1
  ...
  kwargs:
    ...
    kst_entry_hours: [0,1,2,3,5,6,7,8,9,10,11,12,15,18,19,20,21,22,23]  # 19h (2026-06-14 오후 역알파 {4,13,14,16,17} 제외)
