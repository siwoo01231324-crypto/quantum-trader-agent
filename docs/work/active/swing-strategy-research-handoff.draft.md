...
## 확정 결정 (2026-06-30) — 누적 sim + A/D + 유니버스/생존편향

**신호 수익률 누적 sim (라이브 전략 객체 구동, sim==live)**: `_swing_signal_returns_sim2.py`,
`_swing_portfolio_eval.py`. 데이터 = binance_1h 캐시 25 알트→4h (2021~2026, BTC게이트=별도 4h fetch).

**(A) 정직비용 — 둘 다 cost wall 통과**:
- 투매반등: net(10bp) +1.95%/거래, PF 1.63, net+ 5/6년 (2023만 −). net(20bp) +1.85%.
- 돌파/터틀: net(10bp) +1.06%/거래, PF 1.31, net+ 4/6년 (2022·2025 −). net(20bp) +0.96%.
- → 인트라데이 비용벽과 정반대. 거래당 움직임(+1~2%) ≫ 비용. 스윙 가설 5y 확정.

**(D) 사이징 복리 (top_n=8, risk1%, lev1.0)**:
- 투매반등 CAGR +8.5%/MDD 24.6%/Sharpe 1.05. 돌파 CAGR +19.8%/MDD 36.7%/Sharpe 0.70.
- 합성(페어) CAGR +30.5%/MDD **46.0%**/Sharpe 0.94. lev1.5 = CAGR 35%/MDD 56%.
- ⚠️ 페어 MDD 46% 높음 — 단일 basket·공유 top_n8 라 2024 불장 몰빵. **숙제 = 전략별 사이징 버킷 분리 + majors 유니버스**.

**페어 분산 검증 (연도별 단리 합산)**: 한 쪽 죽는 해마다 반대쪽이 받침 —
2022 돌파−199%/투매+53%, 2023 투매−176%/돌파+636%, 2025 돌파−173%/투매+366%. 합산 5/6년 +.

**유니버스/생존편향 확정**:
- 생존편향 노출 비대칭: **돌파=구조적 면역**(EMA200+BTC게이트가 죽는코인 배제), **투매반등=취약**(falling knife).
- → **돌파 = 동적 top-N broad**, **투매반등 = 유동성 메이저 고정**. 공통 제외: 토큰화주식·스테이블·
  레버토큰·신규<60일·저유동. 라이브 `binance_top`/`fetch_futures_24h_snapshot` 메커니즘 재사용.
- 활성화 게이트: 투매반등은 **폐지코인 주입 사망률 테스트**(터틀 선례) 추가 필요.

**남은 구현 순서**: (B 전용패스) 채널청산 loop 배선 — **history_lookup 이 종목 4h 봉(닫힌봉)을 줘야 함.
universe_ohlcv 인터벌이 4h인지 검증 필수**(1h면 Donchian10=10h 오작동, silent 머니버그). env-guard
default-off(byte-identical) + sweep_timeouts 무영향 회귀박제. → (C) /swing 대시보드 sim_cache.
