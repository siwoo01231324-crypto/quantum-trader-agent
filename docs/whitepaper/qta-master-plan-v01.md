...
| # | 상태 | 라벨 | 제목 |
|---|------|------|------|
| #328 | ✅ | bug | feat(live): mark-price feed 가 orchestrator.run_bar 도 깨우도록 wire — testnet aggTrade tick 부재 시 cs-tsmom 발주 미트리거 fix |
| #326 | ✅ | bug | fix(live-scanner): ATR-trail floor 0.3% + breakout buffer 0.1% — 저변동성 churn 차단 |
| #324 | ✅ | bug | fix(live): cs-basket-dispatcher dict/object getattr mismatch — universe-scan 발주 silent skip |
| #240 | 🔄 |  | 비상정지(kill-switch) 결함: 대시보드 버튼이 broker 와 미연결 + close-all 기능 부재 |
